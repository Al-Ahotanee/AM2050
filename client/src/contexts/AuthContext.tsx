...
type AuthContextValue = { user: AuthUser | null; ready: boolean; login: (identifier: string, password: string) => Promise<{ success: boolean; error?: string }>; logout: () => Promise<void> };
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({ user, ready, async login(identifier, password) { const clean = identifier.trim(); const result = await apiClient.request<{ accessToken: string; user: AuthUser }>("/auth/login", { method: "POST", body: { phone: clean, email: clean, identifier: clean, password } }); if (!result.success) return { success: false, error: result.error }; apiClient.setAccessToken(result.data.accessToken); setUser(result.data.user); return { success: true }; }, async logout() { await apiClient.request("/auth/logout", { method: "POST" }); apiClient.setAccessToken(null); setUser(null); } }), [user, ready]);
